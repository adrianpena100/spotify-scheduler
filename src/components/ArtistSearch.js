...
const ArtistSearch = () => {
    const [artistName, setArtistName] = useState(''); // State to store the search input entered by the user
    const [artistData, setArtistData] = useState(null); // State to store the fetched artist's data
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/search/${artistName}`);
        ...
        <div>
            ...
            {artistData && (
                <div>
                    ...
                    <img src={artistData.images[0]?.url} alt={artistData.name} style={{ width: '500px', height: '500px', objectFit: 'cover' }}/>
